...
public class SymmetricAES {
    ...
    public static SecretKeySpec getSecretKeySpec(String strKey) {
        SecretKeySpec keySpec = null;
        try {
...
//            key = sha.digest(key);
    ...
    public static SecretKeySpec getSecretKeySpec(String salt, String strKey) {
        SecretKeySpec keySpec = null;
        try {
            byte[] key = (salt.getBytes("UTF-8") + strKey).getBytes("UTF-8");
            MessageDigest sha = MessageDigest.getInstance("SHA-1");
            key = sha.digest(key);
            key = Arrays.copyOf(key, 16); // use only first 128 bit
            keySpec = new SecretKeySpec(key, "AES");
    ...
    public static byte[] decrypt(byte[] origin, SecretKeySpec keySpec) {
        ...
        try {
            ...
            decodedBytes = aesCipher.doFinal(origin); //ToDo
        } catch (Exception e) {
            Log.e(TAG, e.getMessage(), e);
        }
