...
An example comparing the effect of reconstructing noisy fragments of a raccoon face image using various dictionary learning methods.
...
def plot_image_diff(_image, _reference, _title):
	"""Helper function to display denoising"""
	plt.figure(figsize=(5, 3.3))
	plt.subplot(1, 2, 1)
	plt.title('Image')
	plt.imshow(_image, vmin=0, vmax=1, cmap=plt.cm.gray, interpolation='nearest')
	plt.xticks(())
	plt.yticks(())
	plt.subplot(1, 2, 2)
	difference = _image - _reference
	mse = compare_mse(_reference, _image)
	nrmse = compare_nrmse(_reference, _image)
	psnr = compare_psnr(_reference, _image)
...
transform_algorithms = [
	('MiniBatchDL OMP', 'omp', {'transform_n_nonzero_coefs': 2}),
	('RLS-DLA', '', {}),
	('K-SVD', '', {}),
	('T-MOD', '', {}),
	('K-HOSVD', '', {}),
