function [y,D,usage,nz] = ksvddenoise(params,msgdelta)
...
% single image %
% only for stack of 2-D images
if (isfield(params, 'single') && params.single == true)
    if (p ~= 3)
        error('only support 1-D spatial image array\n');
    else
        p = 2;
    end
end
...
if (isfield(params, 'single') && params.single == true)
    ...
    for kk = 1:sz(p+1)
        ids = cell(p,1);
        [ids{:}] = reggrid(sz(1:p)-blocksize+1, trainnum/sz(p+1), 'eqdist');
        params.data = [params.data, sampgrid(x(:, :, kk), blocksize, ids{:})];
    end
...
if (isfield(params, 'single') && params.single == true)
    for kk = 1:sz(p+1)
        params.x = x(:, :, kk);
        [y(:, :, kk),nz] = ompdenoise2(params,msgdelta);
    end
else
    ...
    elseif (p==2)
      ...
      [y,usage,nz] = ompdenoise3(params,msgdelta);
    end
end
