...
% L. Rosasco, A. Verri, M. Santoro, S. Mosci, and S. Villa. Iterative Projection Methods for Structured Sparsity Regularization. MIT Technical Reports, MIT-CSAIL-TR-2009-050, CBCL-282, 2009.
% J. Bioucas-Dias, M. Figueiredo, ?A new TwIST: two-step iterative shrinkage/thresholding  algorithms for image restoration?, IEEE Transactions on Image Processing, December 2007.
%----------------------------------------------------------------------
%
...
xm2 = x(:, 1);
xm1 = x(:, 1);
prev_f = norm(y-A*x(:, 1), 2)^2+tau1*norm(x(:, 1), 1);
prev_ert = 1;
prev_impv = 0;
for n_it = 2 : nIter;
    
    ert(n_it-1) = norm(y-A*x(:, n_it-1), 2)^2+tau1*norm(x(:, n_it-1), 1);
    impv = 1 - ert(n_it - 1)/prev_ert;
    %fprintf('[IPM_SC] Iteration:%f  Total gap:%f Improvement:%f\n', n_it, ert(n_it - 1), impv);
    ...
    while for_ever
        ...
        if (IST_iters >= 2) || ( TwIST_iters ~= 0)
            ...
            % compute residual
            f = norm(y-A*xm2, 2)^2+tau1*norm(xm2, 1);
            ...
            f = norm(y-A*x_temp, 2)^2+tau1*norm(x_temp, 1);
            if f > prev_f
    ...
    % stops if the improvement is less than 0.00001 two consecutive times
    if ((n_it > 2) && (prev_impv < 0.00001) && (impv < 0.00001))
        break;
    end
    prev_impv = impv;
    prev_ert = ert(n_it - 1);
    
    % stops according to citerion
...
fprintf('[IPM_SC] Iterations:%f  Total gap:%f Last Improvement:%f\n', n_it, prev_ert, prev_impv);
