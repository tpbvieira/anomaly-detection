...
# The data structure to hold our feature extraction functions, which will get applied to each aggregation of the datasets.
extractors = {
...
def agg_time(interval='1S'):
    ...
    pickle_dir = os.path.join('/media/thiago/ubuntu/datasets/network/','stratosphere-botnet-2011/CTU-13-Dataset/','ctu13pickles/')
    path = os.path.join('/media/thiago/ubuntu/datasets/network/','stratosphere-botnet-2011/CTU-13-Dataset/')

    for f in get_files(path):
        strPath = os.path.join(pickle_dir, f.decode('utf-8') + '_pick')
        if strPath.endswith('binetflow_pick'):
            print('Working on ', strPath)
            if not os.path.isfile(strPath):
                # read input file
                df = pd.read_csv(os.path.join(path, f.decode('utf-8')))

                # chop off the ms from the start time and reindex the df to use start time as a datetime object index.
