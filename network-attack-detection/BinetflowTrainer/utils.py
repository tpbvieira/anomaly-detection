...
def save_results(destination_path, file_name, start_time, interval, args, print_contents=False):
...
def get_classifier(classifier):
    if classifier == 'svm':
        ...
        clf = tree.DecisionTreeClassifier(class_weight='balanced', criterion='entropy', splitter='best')
    elif classifier == 'nb':
        clf = GaussianNB()
    elif classifier == 'rf':
        clf = RandomForestClassifier(n_estimators=100, max_features=None, criterion='entropy', n_jobs=2)
    else:
...
def plot_roc_curve(fpr, tpr, roc_auc, title='ROC curve'):
    plt.figure()
    lw = 2
    colors = cycle(['darkorange', 'blue', 'red', 'green', 'black', 'purple', 'yellow'])
    for i, color in zip(range(len(fpr), colors)):
        plt.plot(fpr[i], tpr[i], color=color, lw=lw, label='ROC area {0:.4f}'.format(roc_auc[i]))
    # plt.plot(fpr, tpr, color='darkorange', lw=lw, label='ROC area under curve = {}'.format(roc_auc))
