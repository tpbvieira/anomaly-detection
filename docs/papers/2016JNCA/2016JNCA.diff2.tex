...
\DIFdelendFL \DIFaddbeginFL \includegraphics[height=6cm, width=9cm]{results/figures/fig09.png}
     \DIFaddendFL \caption{Scenario to reproduce legitimate traffic, noise, flood and port scan.}
     \label{fig:fig1}
\end{figure}
...
super-user privileges; and probe attack. Since the proposed approach focus on flood and probe attack, \DIFdelbegin \DIFdel{we analyze }\DIFdelend \DIFaddbegin \DIFadd{the analysis concentrates on }\DIFaddend the attacks that present behaviors similar to flood or probe attack. We observe that the most cases of DoS focus on exploit system vulnerabilities instead of on flooding attack. One example is the occurrence of a neptune attack which sends 20 only SYN packets, what is a behavior that differs of the expected flood attack behavior. Therefore, \DIFdelbegin \DIFdel{we select }\DIFdelend \DIFaddbegin \DIFadd{there were selected }\DIFaddend the cases that simulates several network traffic or numerous connection requests, also known as flooding attack \cite{ahmed2016survey,osanaiye2016distributed}, and the cases that scan ports sending just a few packets. From the simulated probe attacks, we select the cases that rely on TCP or UDP connections.
...
This subsection presents a summarized view of results obtained from the application of the proposed framework, focusing on the largest eigenvalue analysis, model order selection and the eigenvalue analysis, for flood and probe attack detection in the DARPA 1998 dataset. Since the proposed framework is detailed \DIFdelbegin \DIFdel{explained in previous sections, here we focus }\DIFdelend \DIFaddbegin \DIFadd{in Section \ref{sec:prop_getv} and in Subsections \ref{sec:AnalyzedScenario}, \ref{sec:largesteigenvaluesanalysis}, \ref{sec:MOSSchemesEvaluation}, \ref{sec:EigenvalueAnalysis} and \ref{sec:EigenSimilarityAnalysis}, here the focus is }\DIFaddend on the parameter selection, dataset evaluation and results for flood and probe attack identification.

The DARPA dataset includes 7 weeks of sniffed traffic saved into raw packet data. The traffic and the labeled attacks are grouped by week and day, with information of the number and types of attacks per day, but also providing the start time for each labeled attack. For this evaluation, \DIFdelbegin \DIFdel{we performed }\DIFdelend an evaluation per day \DIFaddbegin \DIFadd{was performed}\DIFaddend , considering the network traffic of 24 hours split into $Q$ time frames of 60 minutes ($N = 60$) and \DIFdelbegin \DIFdel{aggregated }\DIFdelend \DIFaddbegin \DIFadd{aggregate }\DIFaddend by minute and by port number. For each time frame $q$, a traffic matrix $\boldsymbol{X}^{(q)} \in \mathbb{R}^{17 \times 20}$ \DIFdelbegin \DIFdel{was }\DIFdelend \DIFaddbegin \DIFadd{is }\DIFaddend obtained, considering the ports 20, 21, 22, 23, 25, 79, 80, 88, 107, 109, 110, 113, 115, 143, 161, 389 and 443.

Since the proposed framework focus on flood and probe attack detection, \DIFdelbegin \DIFdel{we evaluated }\DIFdelend only the attacks with behavior similar to flood or probe attack \DIFdelbegin \DIFdel{. Initially we selected }\DIFdelend \DIFaddbegin \DIFadd{were evaluated. Initially }\DIFaddend all DoS and probe attacks \DIFdelbegin \DIFdel{, but we }\DIFdelend \DIFaddbegin \DIFadd{were selected, but it was }\DIFaddend observed that the most cases of DoS focus on exploit system vulnerabilities instead of flooding attack, and most of probe attacks focus on ICMP intead of port scanning. Therefore, for evaluation of the proposed approach for flood and probe attack detection, it is necessary to select cases that implements flood or port scan behaviors. \DIFdelbegin \DIFdel{We select the }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend following week-day-attack cases  \DIFaddbegin \DIFadd{were selected}\DIFaddend : 
...
\DIFadd{Table \ref{tab:tab12} presents the evaluated results for attack detection. }\DIFaddend The analysis based on sample covariation of zero mean variables \DIFdelbegin \DIFdel{was }\DIFdelend \DIFaddbegin \DIFadd{is }\DIFaddend evaluated for flooding behavior of netpune attacks, obtaining rates of \DIFdelbegin \DIFdel{100}\DIFdelend \DIFaddbegin \DIFadd{100.00 }\DIFaddend \% for true positive (TP) detection and \DIFdelbegin \DIFdel{60}\DIFdelend \DIFaddbegin \DIFadd{60.00 }\DIFaddend \% for false positive (FP) detection from 30 time frames. The results also show \DIFdelbegin \DIFdel{50}\DIFdelend \DIFaddbegin \DIFadd{50.00 }\DIFaddend \% of misclassification rate \cite{bhuyan2014network}, which attempts to estimate the probability of disagreement between the true and predicted cases by dividing the sum of FN and FP by the total number of pairs observed. 
...
The analysis based on covariance of zero mean and unitary standard deviation variables was evaluated for port scan attacks, including probe attacks and DoS attacks that send few packets for several ports in order to exploit some vulnerability. The results show rates of \DIFdelbegin \DIFdel{76.9}\DIFdelend \DIFaddbegin \DIFadd{76.92 }\DIFaddend \% for TP detection and \DIFdelbegin \DIFdel{23}\DIFdelend \DIFaddbegin \DIFadd{18.52 }\DIFaddend \% for FP detection from 94 time frames. The observed misclassification rate for this scenario \DIFdelbegin \DIFdel{was 32.7}\DIFdelend \DIFaddbegin \DIFadd{is 32.73 }\DIFaddend \%. \DIFdelbegin \DIFdel{We }\DIFdelend \DIFaddbegin \DIFadd{It was }\DIFaddend observed that all FN cases are probe attacks with a time delay between scanning one port and start scanning the next port, what can be called as sparse probe attacks. Cases with a delay of one minute or more were not detected by the proposed approach.

The performance of detection rate of flooding attacks is compared with the method proposed by Callegari \emph{et al} \cite{Zonglin2009}. This work is a statistical method, based on PCA, without training or learning methods, even though it relies on visual analysis for principal components selection. The best detection rate of \cite{Zonglin2009} was \DIFdelbegin \DIFdel{82}\DIFdelend \DIFaddbegin \DIFadd{82.00 }\DIFaddend \% for detection of synthetically added flood attacks, while \DIFdelbegin \DIFdel{our proposal obtains 100}\DIFdelend \DIFaddbegin \DIFadd{this current proposal obtains 100.00 }\DIFaddend \% of detection rate for detection of flood attacks of DARPA dataset.

Due to the lack of statistical techniques without training or learning methods for detection of probe attacks, \DIFdelbegin \DIFdel{we compare our approach with }\DIFdelend \DIFaddbegin \DIFadd{this proposed approach is compared to the }\DIFaddend Lu and Ghorbani's \cite{Lu2009} proposal, which is a network anomaly detection model based on signal processing techniques that uses DARPA dataset for evaluation. The results of \cite{Lu2009} show the best detection rate of 94.67 \% in terms of general attack instance detection, but shows a case case with \DIFdelbegin \DIFdel{50}\DIFdelend \DIFaddbegin \DIFadd{50.00 }\DIFaddend \% of attack instance detection for the portsweep attack. The proposed approach presents \DIFdelbegin \DIFdel{76.9}\DIFdelend \DIFaddbegin \DIFadd{76.92 }\DIFaddend \% of detection rate measured specifically for probe attacks, without the requirement of learning or training methods, in contrast to Lu and Ghorbani's \cite{Lu2009} work.
...
\DIFaddend For better understanding the scalability and impact of configurations of $N$, $M$ and $Q$, \DIFdelbegin \DIFdel{we evaluated }\DIFdelend the processing time \DIFdelbegin \DIFdel{requited }\DIFdelend \DIFaddbegin \DIFadd{required }\DIFaddend for different scenarios of parameter configurations and dataset \DIFaddbegin \DIFadd{were evaluated}\DIFaddend , measuring the processing time of: 
