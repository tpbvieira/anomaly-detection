...
%DIF ADD 2016JNCA.tex            Fri Nov  4 16:08:25 2016
 %DIF > 
%DIF -------
...
\DIFadd{Table \ref{tab:tab12} presents the evaluated results for attack detection, considering rates of TP, FP \mbox{%DIFAUXCMD
\cite{fleiss2013statistical}
}%DIFAUXCMD
and misclassification, which is defined as $\frac{(FN+FP)}{(TP+FP+FN+TN)}$ \mbox{%DIFAUXCMD
\cite{bhuyan2014network}
}%DIFAUXCMD
. The analysis based on sample covariation of zero mean variables is evaluated for flooding behavior of netpune attacks, obtaining rates of 100.00 }\% \DIFadd{for true positive (TP) detection and 60.00 }\% \DIFadd{for false positive (FP) detection from 30 time frames. The results also show 50.00 }\% \DIFadd{of misclassification rate , which attempts to estimate the probability of disagreement between the true and predicted cases by dividing the sum of FN and FP by the total number of pairs observed. 
