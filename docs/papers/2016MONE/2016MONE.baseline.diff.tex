...
%DIF ADD 2016MONE.tex            Mon Jan  9 23:17:29 2017
%
...
\author{Tatiana Galibus				\and
        Thiago P. de B. Vieira		\and
        	...
        	Robson \DIFaddbegin \DIFadd{de }\DIFaddend O. Albuquerque		\and
        	João Paulo C. L da Costa		\and
        	Rafael T. de \DIFdelbegin \DIFdel{Souza }\DIFdelend \DIFaddbegin \DIFadd{Sousa }\DIFaddend Júnior	\and
...
\institute{Tatiana Galibus \and Viktor Krasnoproshin \at
           ...
           \and
           Tatiana Galibus \and Thiago P. \DIFaddbegin \DIFadd{de }\DIFaddend B. Vieira \and Robson \DIFaddbegin \DIFadd{de }\DIFaddend O. Albuquerque \and João Paulo C. L da Costa \and Rafael T. de \DIFdelbegin \DIFdel{Souza }\DIFdelend \DIFaddbegin \DIFadd{Sousa }\DIFaddend Júnior  \at
           ...
           João Paulo C. L da Costa \and Giovanni del Galdo \at
				Institute for Information Technology, Ilmenau University of Technology, Ilmenau, Germany\DIFdelbegin %DIFDELCMD < \and
%DIFDELCMD <            %%%
\DIFdel{João Paulo C. L da Costa }%DIFDELCMD < \and %%%
\DIFdel{Giovanni del Galdo }%DIFDELCMD < \at
%DIFDELCMD < 				%%%
\DIFdelend \DIFaddbegin \DIFadd{, and }\DIFaddend Fraunhofer Institute for Integrated Circuits IIS, Erlangen, Germany
           \and
...
\begin{abstract}
Preventing data leakage on the mobile client is a crucial security problem. Therefore, additional control and protection should be taken for the confidential data on the mobile clients that leave the boundaries of the organization. \DIFdelbegin \DIFdel{In this scenario, such apps can be used on the offline mode for the convenience and traffic reducing or even for a particular characteristic of the mobile client or of the business. }\DIFdelend This paper presents a novel approach to the security of the corporate mobile clients, in particular \DIFaddbegin \DIFadd{when they operate in }\DIFaddend the offline mode. The \DIFdelbegin \DIFdel{important part of the presented approach is the }\DIFdelend \DIFaddbegin \DIFadd{presented approach includes the essential }\DIFaddend conceptualization and the definition of the core methodology to solve the problem of offline mobile security, i.e. the protection of the confidential data in use when the mobile client is not connected to the corporate cloud. The protection of the sensitive data is provided by the combination of \DIFdelbegin \DIFdel{the cryptographic }\DIFdelend \DIFaddbegin \DIFadd{cryptographic means }\DIFaddend and analytics methods to detect malicious user behavior. The proposed security architecture supports the basic mobile client protection principles: minimized traffic load and reduced communication with the cloud; usage of \DIFdelbegin \DIFdel{the }\DIFdelend light-weighted operations and an optimized combination of the security methods.
\end{abstract}

\DIFdelbegin %DIFDELCMD < \keywords{ offline mobile cloud security \and offline attribute-based encryption \and model order selection \and user behavior analysis \and secret sharing}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \keywords{offline mobile cloud security \and offline attribute-based encryption \and model order selection \and user behavior analysis \and secret sharing}
\DIFaddend 
...
\DIFaddend The protection scheme used on a mobile device should be both computationally secure as well as resource-constrained due to battery power limitations \cite{khan2015cloud}. Therefore, encrypting files and generating keys on a mobile device is not considered a good solution. On the other hand, the protection schemes with good computational qualities lack the security analysis in many cases \cite{khan2014bss}. The common practice is the shadow user activities monitoring \cite{yovel2014}. However, the mobile device usage stays unprotected in all the proposed scenarios while in offline mode.
...
From the theoretical point of view of this problem, there are several surveys, whose common point is the mobile cloud as a rapidly developing paradigm that poses many security and complexity problems \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{yovel2014, khan2015cloud, khan2014survey, khan2013towards}
}%DIFAUXCMD
. }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{khan2013towards, yovel2014, khan2014survey, khan2015cloud, mayrhofer2015architecture, chang2015design, xia2015tinman, kulkarni2015addressing, shila2016amcloud, heydari2016efficient}
}%DIFAUXCMD
. Kulkarni and Khanai \mbox{%DIFAUXCMD
\cite{kulkarni2015addressing}
}%DIFAUXCMD
discuss the most important threats related to Mobile Cloud Computing and argue that there is a need for a lightweight secure framework that provides security with minimum communication and processing overhead on mobile devices. 
}

\DIFaddend An analysis of the new models of mobile cloud computing and new ways of using data storage services is presented in \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{khan2015cloud, khan2013towards}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{khan2015cloud, khan2013towards, mayrhofer2015architecture, chang2015design}
}%DIFAUXCMD
}\DIFaddend . Commonly, \DIFdelbegin \DIFdel{all }\DIFdelend the models and protection schemes concentrate on the encryption properties and either perform the computations on their own \cite{zhao2010trusted, yang2011provable} or use the cloud provider to off-load the cryptographic operations \cite{itani2010energy, ren2011lightweight}. Obviously, it is natural the mobile client cannot handle all operations securely without the assistance of a cloud provider, due to resources constraint and battery limitation.

The necessity to use schemes that function without putting load on a provider arises when it is desired to make the mobile client less dependent on the cloud provider, i.e. corporate client continues to provide the secure access to the sensitive data without connection to the network. As discussed in \cite{khan2015cloud}, all the currently known schemes of encryption, performing the computation, either use a cloud provider \cite{zhao2010trusted}, a third party trusted agent \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{yang2011provable}
}%DIFAUXCMD
or }\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{yang2011provable, heydari2016efficient}
}%DIFAUXCMD
, }\DIFaddend a combination of both \cite{itani2010energy} \DIFaddbegin \DIFadd{or ad hoc approaches \mbox{%DIFAUXCMD
\cite{shila2016amcloud}
}%DIFAUXCMD
}\DIFaddend . In some cases, they concentrate on computational complexity without taking care of user privacy and security \cite{ren2011lightweight}. Therefore, according to \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{khan2015cloud, tenorio2013greatest, khan2013towards}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{khan2013towards, khan2015cloud}
}%DIFAUXCMD
}\DIFaddend , the state-of-the-art mobile cloud security models do not consider the problem of the offline security mode. 
...
\DIFaddend The core set of functions and protocols can be divided into three sets of operations as shown in Figure \ref{fig:01}.
...
\DIFdelbegin \DIFdel{These operations function based on }\DIFdelend \DIFaddbegin \DIFadd{Figure \ref{fig:01} describes }\DIFaddend the mobile client \DIFdelbegin \DIFdel{architecture, which is depicted in Figure \ref{fig:02}}\DIFdelend \DIFaddbegin \DIFadd{protection both in online and offline mode. In online mode, the client has the possibility to connect to the server and the security of the client is enhanced by the server-backed up mechanisms. On the other hand, in offline mode the client’s security is supported by the standalone mechanisms. Additionally, the mobile client protection is enhanced by the threat intelligence unit providing the constant monitoring and analysis.
}
...
The \DIFdelbegin \DIFdel{infrastructure in Figure \ref{fig:03} }\DIFdelend \DIFaddbegin \DIFadd{Figure \ref{fig:03} describes the proposed architecture for offline mobile security, showing the modules responsible for securing the mobile client, which }\DIFaddend includes:
...
The UAL is responsible for the incremental logging \DIFaddbegin \DIFadd{of activities of the mobile client}\DIFaddend , feature extraction and data modeling for malicious behavior detection, through the Log Analysis Center (LAC). \DIFaddbegin \DIFadd{As an internal module of the mobile client, the UAL implements monitors of selected events of the application, such as a login attempt or a file decryption, and logs the desired information for further analysis. }\DIFaddend The logged information shall be decomposed into selected features and modeled as matrices, composed of the number of occurrences of the selected features by its location and by time. The resultant data is \DIFdelbegin \DIFdel{submited }\DIFdelend \DIFaddbegin \DIFadd{submitted }\DIFaddend to the LAC, for anomaly detection.
...
. Here $w \in \mathbb{Z}$ is the unique security parameter allowing to control the key generation for each user group.
	...
	\item Encrypted text $M$, in \DIFdelbegin \DIFdel{our }\DIFdelend \DIFaddbegin \DIFadd{this }\DIFaddend context, $M = FILE\_KEY$, or the permanent AES symmetric key, which allows to avoid the file re-encryption. \DIFdelbegin \DIFdel{Encryption procedure is }\DIFdelend \DIFaddbegin \DIFadd{The encryption procedure is performed by a }\DIFaddend multiplication. The set of the public keys $E_i$ (PUBLIC\_SHARE\_KEY) corresponding to the set of groups able to access the text is kept along with the encrypted text $E$: \DIFdelbegin \DIFdel{$E=Me(g,g)^{ys}, \{E_i = g^{\frac{t_is}{w}}\}, \forall i \in \{t_i\}_M$}\DIFdelend \DIFaddbegin \DIFadd{$E=Me(g,g)^{ys}, \{E_i = g^{\frac{t_is}{w}}\}, \forall i, t_i \in \{t_i\}_U$. Here $s \in \mathbb{Z}$ is the unique security parameter allowing to control the key generation for each file share}\DIFaddend .
...
The attribute-based private keys $D_i$ should be protected while being stored in the device memory. Therefore, server encrypts the set of \DIFdelbegin \DIFdel{$Di$ }\DIFdelend \DIFaddbegin \DIFadd{$D_i$ }\DIFaddend with a single AES key before sending it to the user device. This AES key (master key) is denoted by the value KEY\_SET\_KEY in our notation. KEY\_SET\_KEY is a secret value and it is split by the secure method of polynomial modular secret sharing \cite{galibus2015mobile, galibus2008some} into the set of 4 shares: KEY\_SET\_KEY = PASS + PIN + TIME + DEV\_PASS. 

The adversary cannot get any information of the KEY\_SET\_KEY unless he \DIFdelbegin \DIFdel{possessing }\DIFdelend \DIFaddbegin \DIFadd{possesses }\DIFaddend all 4 key parts. Here the values PASS and PIN are predefined similar to the construction in \cite{galibus2015mobile}.
...
The set of obtained vectors $\mathbf{y}_{m}^{(q)}$ composes the zero mean matrix $\mathbf{Y}^{(q)}$, then the zero mean covariance matrix $\mathbf{\hat{R}}_{yy}^{(q)}$ can be \DIFdelbegin \DIFdel{calculated }\DIFdelend \DIFaddbegin \DIFadd{estimated }\DIFaddend as follows

\begin{equation}\label{eq:eq04}
\mathbf{\hat{R}}_{yy}^{(q)} = \frac{1}{N}\mathbf{Y}^{(q)}\mathbf{Y}^{(q)^{\rm T}}\DIFdelbegin \DIFdel{.
...
\begin{enumerate}
	...
	\item \DIFdelbegin \DIFdel{Does nothave possibility to perform }\DIFdelend \DIFaddbegin \DIFadd{Steal the encrypted files.
}\end{enumerate}
...
\begin{enumerate}
	...
	\item Steal the \DIFdelbegin \DIFdel{encrypted files but not the decrypted ones}\DIFdelend \DIFaddbegin \DIFadd{decrypted files}\DIFaddend .
\end{enumerate}
...
In offline mode, the user is still allowed to get access to operations that do not require communication with the server side. These operations and their selected features shall be incrementally logged by the \DIFaddbegin \DIFadd{UAL of the }\DIFaddend mobile client, in order to be \DIFdelbegin \DIFdel{evaluated }\DIFdelend \DIFaddbegin \DIFadd{analyzed by the TIM }\DIFaddend to identify malicious behaviors. 
\DIFaddbegin 
...
It is easy to check that in the offline mode the mobile client does not perform complex calculations and does not use the resources extensively due to the fact that the initial key is shared and the mobile client only performs decryption, which is not a time-consuming operation. Thus, the proposal supports the concept of the light-weighted client, i.e. the most consuming \DIFdelbegin \DIFdel{operations }\DIFdelend \DIFaddbegin \DIFadd{actions }\DIFaddend are ABE and AES decryption. 

Similarly, in the online mode the mobile client does not perform resource-consuming \DIFdelbegin \DIFdel{operations }\DIFdelend \DIFaddbegin \DIFadd{actions }\DIFaddend apart from J-PAKE construction which is used to renew the user/app master key (DEV\_PASS):

\begin{enumerate}
	\item Generates keys for J-PAKE $\rightarrow$ this can be a resource-consuming \DIFdelbegin \DIFdel{operation}\DIFdelend \DIFaddbegin \DIFadd{action}\DIFaddend . In the future we suggest to replace J-PAKE with SIS-based PKE \cite{bogos2013primeless,bellovin1992encrypted};
	\item Sends and receives data in the clear;
	\item Performs \DIFdelbegin \DIFdel{operations }\DIFdelend \DIFaddbegin \DIFadd{actions }\DIFaddend 1)-7);
	\item Performs the \DIFdelbegin \DIFdel{operation }\DIFdelend \DIFaddbegin \DIFadd{action }\DIFaddend 8) and sends the log data for server side analysis.
\end{enumerate}
...
The authors thank the Brazilian research and innovation agencies \DIFaddbegin \DIFadd{CAPES (Grant FORTE }\\\DIFadd{23038.007604/2014-69), }\DIFaddend FAPDF (Research Support Foundation of the Federal District), FINEP (\DIFdelbegin \DIFdel{Agreement RENASIC / }\DIFdelend \DIFaddbegin \DIFadd{Grant RENASIC }\DIFaddend PROTO 01.12.0555.00), \DIFdelbegin \DIFdel{CAPES and CNPq under the FORTE Project - CAPES Forensic Sciences Announcement 25/2014, under the productivity grant number }\DIFdelend \DIFaddbegin \DIFadd{CNPq (Productivity grant }\DIFaddend 303905/2014-0 \DIFdelbegin \DIFdel{, under the PVE project number }\DIFdelend \DIFaddbegin \DIFadd{and PVE Grant }\DIFaddend 88881.030392/2013-01\DIFdelbegin \DIFdel{for their financial support on this research, and under the project number }\DIFdelend \DIFaddbegin \DIFadd{), as well as the European CSF - Aerospace Technology Program (PDE scholarship within project }\DIFaddend 207644/2015-2\DIFdelbegin \DIFdel{for the PDE scholarship in the context of the program CSF - Aerospace Technology}\DIFdelend \DIFaddbegin \DIFadd{), for their support to this research}\DIFaddend .\end{acknowledgements}
