...
Botnets have emerging as a key threat for cyber security, considering that Botnets are the key platform for many Internet attacks (i.e. spam, distributed denial-of-service (DDoS), identity theft, and phishing). Most of the current botnet detection approaches work only on specific botnet command and control (. Lots of efforts are directed to avoid attacks from known attackers, however attacks from one source is not real nowadays, since the attacks are usualy from distributed botnets that change their approaches frequently, in order to avoid behavioral or pattern based detecions. This changing behavior is known as adversarial model \cite{gu2008botminer, garcia2014empirical,khattak2015botflex,acarali2016survey,wang2017botnet,da2018online,Wang2018ddosbotnetssurvey}.
...
\cite{da2018online} (Online detection of Botnets on Network Flows using Stream Mining) Ver se podemos calcular o F1 e comparar os resutados, mas não seria justo dado que a abordagem deles nao implementa o mesmo sampling e é supervisionado
