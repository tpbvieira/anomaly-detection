...
kddTrainingTarget = kddTraining$attacks

# Alternatively method for categorical values:
...
tuneOutSVM <- tune.svm(as.factor(attacks)~., data=kddTraining, 
...
svmClassifier=svm(as.factor(kddTraining$attacks)~ ., data=kddTraining,
...
predicted$Actual=c(as.character(kddTesting$attacks))
