% ex132           RLS-DLA with adaptive increasing lambda. 
%   1   - AR(1) signal, s=4, N=16, K=32
%    3  - experiment with RLS-DLA and increasing lambda
%     2 - number 2, plot average for 20 trials 200 iterations
%
% To generate new data, make sure to delete or rename 'ex132.mat',
...
% lambda as given by workspace variable 'par' which is set in this file.
...
% 
% HISTORY:  dd.mm.yyyy
% Ver. 1.0  25.06.2009  KS: updated the default values of ex131 
...
if exist(ResultFile,'file')
    ...
    noIT = 200;
    noEX = 20; 
    disp(['noIT = ',int2str(noIT),', noEX (trials) = ',int2str(noEX)]);
    disp('Use default set of adaptive schemes for lambda, variable par.');
    par = cell(3,1);
    par{1} = struct('lamM','C','lam0',0.99,'a',0.25);
    par{2} = struct('lamM','C','lam0',0.99,'a',0.50);
    par{3} = struct('lamM','C','lam0',0.99,'a',0.75);
    %
