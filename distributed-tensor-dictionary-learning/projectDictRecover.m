...
filePath = '/media/thiago/ubuntu/datasets/dictRecovery/results/';           % change to save generated data
s = 5;                                                                      % sparseness
...
%% plot configuration
epsName = sprintf('%s/s=%1i_snr=%li_L=%li_noIt=%li_N=%li_K=%li.eps', filePath, s, snr, L, noIt, N, K);
...
y1=get(gca,'ylim');
hold on
plot([betalim betalim],y1, '-.or')


ylabel('Identified atoms');
xlabel('Degrees rates.');
