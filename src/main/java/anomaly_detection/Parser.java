...
public class Parser {
	...
	public static void main(String[] args) { 
		...
		HashMap<Long,HashMap<String,Integer>> values = parseCSV(filePath, sdf, matrixRowIndeces);
		...
		Matrix[] matrices = modelIntoMatrices(windowSize, matrixRowIndeces, values, matrixColumnIndeces);
		endTime = System.currentTimeMillis();
	...
	private static double[][] getLargestEigValCor(Matrix[] matrices) {
		...
		for(int i = 0; i < matrices.length; i++){
			...
			largestEigValCor[0][i] = MatrixUtil.max(eigVal);
		}
		return largestEigValCor;
	}

	private static double[][] getLargestEigValCov(Matrix[] matrices) {
		double[][] largestEigValCov = new double[1][matrices.length];
		for(int i = 0; i < matrices.length; i++){
			EigenvalueDecomposition eig = EigenAnalysis.eigencovariance(matrices[i]);
			Matrix eigVal = eig.getD();
			largestEigValCov[0][i] = MatrixUtil.max(eigVal);
		}
		return largestEigValCov;
	}

	private static Matrix[] modelIntoMatrices(short windowSize, HashMap<String, Integer> matrixRowIndeces,
